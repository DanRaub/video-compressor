...
async function decodeWithBrowser(bytes, type) {
  const bmp = await createImageBitmap(new Blob([bytes], { type }), { imageOrientation: "from-image", premultiplyAlpha: "none", colorSpaceConversion: "none" });
...
self.onmessage = async ({ data: job }) => {
  ...
  try {
    ...
    self.postMessage({ id, ok: true, bytes: result.buffer, type: kept ? type : outType, kept }, [result.buffer]);
  } catch (err) {
